...
test "queue close wakes consumer — returns null" {
    ...
    const handle = try std.Thread.spawn(.{ .stack_size = 512 * 1024 }, struct {
...
test "bus multiple inbound producers" {
    ...
    for (0..num_threads) |t| {
        handles[t] = try std.Thread.spawn(.{ .stack_size = 512 * 1024 }, struct {
...
test "bus stress: 10 producers × 100 messages" {
    ...
    for (0..num_threads) |t| {
        producers[t] = try std.Thread.spawn(.{ .stack_size = 512 * 1024 }, struct {
            fn run(b: *Bus, tid: usize, a: Allocator) void {
                for (0..msgs_per_thread) |i| {
                    var id_buf: [32]u8 = undefined;
                    const id_str = std.fmt.bufPrint(&id_buf, "{d}:{d}", .{ tid, i }) catch "?";
    ...
    var count: usize = 0;
    const consumer = try std.Thread.spawn(.{ .stack_size = 512 * 1024 }, struct {
