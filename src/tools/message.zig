...
test "MessageTool execute with explicit channel overrides default" {
    ...
    const parsed = try root.parseTestArgs("{\"content\":\"hi\",\"channel\":\"slack\",\"chat_id\":\"room1\"}");
    defer parsed.deinit();
    const result = try mt.execute(testing.allocator, parsed.value.object);
    try testing.expect(result.success);
    try testing.expect(std.mem.indexOf(u8, result.output, "slack") != null);
    testing.allocator.free(result.output);

    var msg = event_bus.consumeOutbound().?;
    defer msg.deinit(testing.allocator);
    try testing.expectEqualStrings("slack", msg.channel);
