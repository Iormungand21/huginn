...
test "dispatcher runs in a separate thread" {
    ...
    const thread = try std.Thread.spawn(.{ .stack_size = 512 * 1024 }, runOutboundDispatcher, .{
...
test "dispatcher concurrent producers + single dispatcher" {
    ...
    // Spawn dispatcher
    const dispatcher = try std.Thread.spawn(.{ .stack_size = 512 * 1024 }, runOutboundDispatcher, .{
        allocator, &event_bus, &reg, &stats,
    });
    ...
    for (0..num_producers) |i| {
        producers[i] = try std.Thread.spawn(.{ .stack_size = 512 * 1024 }, struct {
